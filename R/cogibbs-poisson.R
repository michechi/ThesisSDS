#' Collapsed Gibbs Sampler with Zero-Truncated Poisson Prior
#'
#' Runs a collapsed Gibbs sampler for the MFM-SBM model using a
#' zero-truncated Poisson prior on the number of components.
#'
...
#' @param logVn_miller Pre-computed vector of `log(V_n(t))` values
#'   (from [log_vn_miller()]).
...
#' lv <- log_vn_miller(1, 20, 30)
#' z0 <- c(sample(1:3, 3, replace = FALSE), sample(1:3, 17, replace = TRUE))
#' fit <- cogibbs_poisson(50, 3, sbm$A0, 20, 1, 1, 1, lv, z0)
#' }
cogibbs_poisson <- function(M, K_start, A, n, a, b, gamma_, logVn_miller, z_start) {
  ...
  for (iter in seq_len(M)) {
    ...
    for (i in seq_len(n)) {
      ...
      pr_tbl <- c(pr_tbl,
                   exp(logVn_miller[c_i + 1] - logVn_miller[c_i]) * gamma_ * mAi)
